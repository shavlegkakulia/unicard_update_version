...
namespace Kunikardus.Droid.Plugins.SlidingUpPanel
{
    public class SlidingUpPanelLayout : ViewGroup
    {
        ...
        private const int DefaultPanelHeight = 10;
        ...
        public event SlidingUpPanelSlideEventHandler PanelExpandedFromSlide;
        ...
        {
            get { return _slideState == SlideState.Collapsed; }
        }
            ...
            switch (action & MotionEventCompat.ActionMask)
            {
                ...
                case (int)MotionEventActions.Up:
                {
                    ...
                    if (dx * dx + dy * dy < slop * slop && IsDragViewUnder((int)x, (int)y))
                    {
                        ...
                        {
                            _slideState = SlideState.Expanded;
        ...
        {
            if (_slideState == SlideState.Expanded)
            {
                return false;
            }
            //else if(_slideState == SlideState.Anchored)
            //{
            //    return false;
            //}
            var dragView = _dragView ?? _slideableView;
        ...
        {
            OnPanelCollapsed(_slideableView);
        ...
        {
            _slideState = SlideState.Expanded;
            return ExpandPane(0.13f);
        }

        public bool ExpandPane(float slideOffset)
        {
            OnPanelExpanded(_slideableView);
            if (!PaneVisible)
                ShowPane();
        ...
        private class DragHelperCallback : ViewDragHelper.Callback
        {
            ...
            public override void OnViewReleased(View releasedChild, float xvel, float yvel)
            {
                ...
                {
                    top = 378;
                    if (_panelLayout.PanelExpandedFromSlide!=null)
                    {
                        _panelLayout.PanelExpandedFromSlide(this, new SlidingUpPanelSlideEventArgs { Panel = _panelLayout._slideableView, SlideOffset = _panelLayout._slideOffset });
                    }
                    _panelLayout.ExpandPane();
                    return;
                }
        ...
        {
            return FloatNearlyEqual(a, b, 0.2f);
        }
    }
}
